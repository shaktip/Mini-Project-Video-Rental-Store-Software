...
$sql = "SELECT * FROM courier_order co JOIN item_master itm ON co.item_id = itm.item_id ".
        " JOIN member m ON co.placed_by = m.member_id WHERE co.status='ORDERED' AND co.order_for = 'RENT' ORDER BY order_date";
$res = $conn->query($sql);
if(!$res || $res->num_rows == 0){
    echo "<br/><br/><h3 style='color:blue'>No rent orders are pending to be processed as of now!</h3>";
    require('include/page_footer.php');
    exit();
}
...
?>
    <form onsubmit="return validate_chk();" method="post" action="process_order2.php" name="process_order" id="process_order">

<h2 class="form-signin-heading" align="center"> Rent Orders Placed</h2>
<br/>
        <table class="table">
            <tr class="info">
                <td valign="middle">
                ...
                <td>
                    <input class="btn btn-info" type="submit" name="process_order" value="Process Orders"/>
                </td>
            </tr>
    ...
    <table class="table table-bordered table-hover">
        <thead>
            <tr class="success">
                <th/><th>Order ID</th><th>Title</th><th>Order Date</th><th>Delivery Address</th><th width="120px"># Copies Available in Store</th><th>Member ID</th>
...
while ($row = $res->fetch_array()) {
    ...
    $res1 = get_rows($conn,"item_details", "count(copy_id)", "item_id=" . $row["item_id"] . " AND availability=1 AND status='A' AND store_id=" . $_SESSION["store_id"]);
    $num_of_copies = ($res1->fetch_array())[0];
    ...
    if($num_of_copies == 0)
        echo '<td><input type="checkbox" disabled name="item_list[]" value="' . $row["order_id"] . '"/> ';
    else
    ...
    echo ($num_of_copies==0)?"<span style='color:red'>" .$num_of_copies . "</span>" : "<span style='color:blue'>" .$num_of_copies . "</span>";
