...
if($_SESSION['role'] == 'member' || $_SESSION['role'] == 'S' ) {
    ...
    <form onsubmit="return fn_search_mem();" action="<?php $_PHP_SELF ?>" method="post" name="search_mem"
        ...
        <h2 class="form-signin-heading" style="text-align:center; width:700px;">Order Item on Call</h2>
                ...
                <tr>
                    <td>
                        ...
                        <input value="<?php echo isset($_POST['user_name']) ? $_POST['user_name'] : '' ?>"
                               class='input-sm' type="text" name="user_name" id="user_name" autofocus>
                    </td>
                    <td> /</td>
                    <td>
                        ...
                        <input value="<?php echo isset($_POST['mobile_number']) ? $_POST['mobile_number'] : '' ?>"
                               class='input-sm' type="text" name="mobile_number" id="mobile_number" pattern="[0-9]*" title="Only digits"
                               autofocus>
                    </td>
                    <td>
            ...
            if ($result) {
                ...
                if (isset($_SESSION['error']) && $_SESSION['error'] != '') {
                    ...
                    if ($_SESSION['result_exists'] == 'n') {
                        echo '<div class="txt-heading" ><b>No items issued to the member as of now </b><a id="btnEmpty" href="order_item.php?id=' . $row['member_id'] . '" name="issue_item"  ><b>Proceed to place order</b></a></div>';
                    } else {
                        if ($_SESSION['result_exists'] == 'y') {
                            echo '<div class="txt-heading" ><b>Items already issued to the member </b><a id="btnEmpty" href="order_item.php?id=' . $row['member_id'] . '" name="issue_item"  ><b>Proceed to place order</b></a></div>';
                        } else {
                            echo '<div class="txt-heading" ><b>No more items can be issued to the member</div>';
                        }
    ...
    <script>
        ...
        function get_selected_members_info(rad) {
            ...
            if (rad.value == '') {
                ...
                xmlhttp.open("GET", "get_issued_items4.php?id=" + rad.value, true);
                xmlhttp.send();
            }
